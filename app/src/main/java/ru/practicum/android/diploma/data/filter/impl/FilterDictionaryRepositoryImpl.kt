...
class FilterDictionaryRepositoryImpl(private val networkClient: NetworkClient) : FilterDictionaryRepository {
    ...
    override fun getRegions(): Flow<Resource<List<RegionModel>>> =
        networkClient.doRequest<GetRegionRequest, List<RegionDto>>(GetRegionRequest).mapResource { responseDto ->
    ...
    private fun <T, X> Flow<Resource<T>>.mapResource(transform: (T) -> X): Flow<Resource<X>> = map { resource ->
        when (resource) {
